...
HighLight.prototype.det4 = function(x11,  x12,  x13,  x14,
                            		x21,  x22,  x23,  x24,
                            		x31,  x32,  x33,  x34,
                            		x41,  x42,  x43,  x44) {
...
HighLight.prototype.Solve4 = function(A, x) {
	var B = new Array(16);
	var i,j;
	var y = new Array(4);
	var det, dem;

	det = this.det4(A[0], A[4], A[8], A[12],
                    A[1], A[5], A[9], A[13],
                    A[2], A[6], A[10], A[14],
                    A[3], A[7], A[11], A[15]);
	...
	for(i=0;i<4;i++) {
		...
		dem = this.det4(B[0], B[4], B[8], B[12],
                        B[1], B[5], B[9], B[13],
                        B[2], B[6], B[10], B[14],
                        B[3], B[7], B[11], B[15]);
		y[i] = dem/det;
...
HighLight.prototype.eval_highlight = function(highlight_type, patch, funcs) {
	...
	for(var i = 0; i < patch.geometry.faces.length; i++)
	{
		...
		for (var j = 0; j < 4; j++){
			...
			if(highlight_type == this.HIGHLIGHTLINE) {
				func = this.calc_D( P, N, A, H)/hl_step;
