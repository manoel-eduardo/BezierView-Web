...
EVal.prototype.eval_patch = function(patch, subDepth){
	...
	switch(type) {
		...
		case 3:		// triangular bezier
			return this.eval_triangular(patch, subDepth);
			break;
		case 4:		// tensor-product
		case 5: 
		case 8:
			return this.eval_tensor_product(patch, subDepth);
			break;
		default:
			break;
...
EVal.prototype.eval_control_mesh = function(type, degs,vecs){
	var size;
	var sizeu, sizev, bigstepu, bigstepv, degu, degv;
	...
	if (type == 3) {	// Triangular Bezier
		...
		for (var i = 0; i < size; i++){
			geo.vertices.push(new THREE.Vector4(vecs[i].x, vecs[i].y, vecs[i].z, 1.0));
		}
		...
		for(i=0; i<=d;i++) {
			for(j=0; j<=d-i;j++) {
				k = d-i-j;
				var v1 = this.b2i_i(i+1,j,k,deg);
				var v2 = this.b2i_i(i,j+1,k,deg);
				var v3 = this.b2i_i(i,j,k+1,deg);
		...
		for(var i = 0; i < size; i++){
			if(type == 8) // rationize the vertice
				geo.vertices.push(new THREE.Vector4(vecs[i].x/vecs[i].w, vecs[i].y/vecs[i].w, vecs[i].z/vecs[i].w, vecs[i].w/vecs[i].w));
			else
				geo.vertices.push(new THREE.Vector4(vecs[i].x, vecs[i].y, vecs[i].z, 1.0));
		}
...
/** Evaluates triangular bezier patches
	Type 3 
	**/
EVal.prototype.eval_triangular = function(patch, subDepth) {
	var i,j,k;
    var m;
    ...
    for (var uu=0; uu<=pts; uu++)
    {
        for (var vv=0;vv<=pts-uu;vv++)
        {
            ...
            this.evalPN(V00, V01, V10, eval_P[loc], eval_N[loc]);
...
/** Evaluates tensor-product patches
	Types 4,5 and 8 **/
EVal.prototype.eval_tensor_product = function(patch, subDepth) {
	var type = patch.type;
	var vecs = patch.pts;
	...
	this.BBcopy4( vecs, degu, degv, pts, bb);

	// subdivision
	for (var i=0; i <subDepth; i++)
	{
		this.RSubDiv(bb, st, degu, degv, sizeu,sizev);
...
EVal.prototype.b2i_i = function(i, j, k, d) {
    var lk = 0;
    var kk = 0;
...
*/
EVal.prototype.b2i_j = function (i, j, k, d) {
    var lk = 0;
    var kk = 0;
...
//
EVal.prototype.b2i_k = function(i, j, k, d) {
    var lk = 0;
    var kk = 0;
    k = d-i-j;
...
EVal.prototype.BBcopy4 = function( buf, degu, degv, st, bb) {
	var i,j;
	var C;
...
EVal.prototype.RSubDiv = function(bb, step, degu, degv, sizeu, sizev) {
	var 	m,k,l;
	var 	row,col,C;
...
// Cross product 
EVal.prototype.VVcross = function(v1, v2) {
...
EVal.prototype.evalPN = function(v00, v01, v10, P, N) {
